...
class StudentMigrate(models.TransientModel):
    ...
    course_from_id = fields.Many2one('op.course', 'Del curso', required=True)
    course_to_id = fields.Many2one('op.course', 'Cursar', required=True)
    batch_id = fields.Many2one('op.batch', 'Para Lote')
    optional_sub = fields.Boolean("Materias opcionales")
    student_ids = fields.Many2many(
        'op.student', string='Estudiante(s)', required=True)
    ...
    def _check_admission_register(self):
        for record in self:
            if record.course_from_id == record.course_to_id:
                raise ValidationError(_("Desde Curso no debe ser igual \
                como al curso!"))
            ...
            if record.course_from_id.parent_id:
                if record.course_from_id.parent_id != \
                    ...
                    raise ValidationError(_(
                        "No se puede migrar, como lo hacen los cursos seleccionados \
                        Comparte el mismo curso de padres!"))
            else:
                raise ValidationError(_(
                    "No se puede migrar, proceder a una nueva admisión"))
    ...
    def student_migrate_forward(self):
        for record in self:
            ...
            act_type = activity_type.search(
                [('name', '=', 'Migración')], limit=1)
            if not act_type:
                act_type = activity_type.create({'name': 'Migración'})
