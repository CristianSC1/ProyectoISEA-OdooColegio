...
class OpSession(models.Model):
    ...
    name = fields.Char(compute='_compute_name', string='Nombre', store=True)
    timing_id = fields.Many2one(
        'op.timing', 'Sincronización', required=True, track_visibility="onchange")
    start_datetime = fields.Datetime(
        'Fecha inicial', required=True,
        default=lambda self: fields.Datetime.now())
    end_datetime = fields.Datetime(
        'Fecha final', required=True)
    course_id = fields.Many2one(
        'op.course', 'Curso', required=True)
    faculty_id = fields.Many2one(
        'op.faculty', 'Facultad', required=True)
    batch_id = fields.Many2one(
        'op.batch', 'Lote', required=True)
    subject_id = fields.Many2one(
        'op.subject', 'Sujeto', required=True)
    classroom_id = fields.Many2one(
        'op.classroom', 'Salon')
    color = fields.Integer('Color Index')
    type = fields.Char(compute='_compute_day', string='Día', store=True)
    state = fields.Selection(
        [('draft', 'Borrador'), ('confirm', 'Confirmar'),
         ('done', 'DoHechone'), ('cancel', 'Cancelado')],
    ...
    def notify_user(self):
        for session in self:
            template = self.env.ref(
                'crmjj_calendario.session_details_changes',
