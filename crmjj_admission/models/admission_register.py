...
class OpAdmissionRegister(models.Model):
    ...
    name = fields.Char(
        'Nombre', required=True, readonly=True,
        states={'draft': [('readonly', False)]})
    start_date = fields.Date(
        'Fecha de inicio', required=True, readonly=True,
        default=fields.Date.today(), states={'draft': [('readonly', False)]})
    end_date = fields.Date(
        'Fecha final', required=True, readonly=True,
        default=(datetime.today() + relativedelta(days=30)),
        states={'draft': [('readonly', False)]})
    course_id = fields.Many2one(
        'op.course', 'Curso', required=True, readonly=True,
        states={'draft': [('readonly', False)]}, track_visibility='onchange')
    min_count = fields.Integer(
        'Número mínimo de admisión', readonly=True,
        states={'draft': [('readonly', False)]})
    max_count = fields.Integer(
        'Número máximo de admisión', readonly=True,
