...
class OpAdmission(models.Model):
    ...
    name = fields.Char(
        'Nombre', size=128, required=True,
        states={'done': [('readonly', True)]})
    batch_id = fields.Many2one('op.batch', 'Lote', )
    middle_name = fields.Char(
        'Primer Nombre', size=128,
        states={'done': [('readonly', True)]})
    last_name = fields.Char(
        'Segundo Apellido', size=128, required=True,
        states={'done': [('readonly', True)]})
    title = fields.Many2one(
        'res.partner.title', 'Title', states={'done': [('readonly', True)]})
    application_number = fields.Char(
        'Numero de aplicacion', size=16, required=True, copy=False,
        states={'done': [('readonly', True)]},
        default=lambda self:
        self.env['ir.sequence'].next_by_code('op.admission'))
    admission_date = fields.Date(
        'Fecha de admisión', copy=False,
        states={'done': [('readonly', True)]})
    application_date = fields.Datetime(
        'Fecha de aplicacion', required=True, copy=False,
        states={'done': [('readonly', True)]},
        default=lambda self: fields.Datetime.now())
    birth_date = fields.Date(
        'Fecha de nacimiento', required=True, states={'done': [('readonly', True)]})
    course_id = fields.Many2one(
        'op.course', 'Curso', required=True,
        states={'done': [('readonly', True)]})
    
    street = fields.Char(
        'Calle', size=256, states={'done': [('readonly', True)]})
    street2 = fields.Char(
        'Calle2', size=256, states={'done': [('readonly', True)]})
    phone = fields.Char(
        'Telefono', size=16, states={'done': [('readonly', True)]})
    mobile = fields.Char(
        'Mobil', size=16, states={'done': [('readonly', True)]})
    email = fields.Char(
        'Email', size=256, required=True,
        states={'done': [('readonly', True)]})
    city = fields.Char('Ciudad', size=64, states={'done': [('readonly', True)]})
    
    state_id = fields.Many2one(
        'res.country.state', 'Estado', states={'done': [('readonly', True)]})
    country_id = fields.Many2one(
        'res.country', 'Pais', states={'done': [('readonly', True)]})
    fees = fields.Float('Matrícula', states={'done': [('readonly', True)]})
    image = fields.Binary('image', states={'done': [('readonly', True)]})
    state = fields.Selection(
        [('draft', 'Borrador'), ('submit', 'Submitted'),
         ('confirm', 'Confirmed'), ('admission', 'Admisión Confirmar'),
         ('reject', 'Rejected'), ('pending', 'Pending'),
         ('cancel', 'Cancelado'), ('done', 'Hecho')],
    ...
    prev_institute_id = fields.Many2one(
        'res.partner', 'Colegio anterior',
        states={'done': [('readonly', True)]})
    prev_course_id = fields.Many2one(
        'op.course', 'Curso anterior', states={'done': [('readonly', True)]})
    prev_result = fields.Char(
        'Resultado anterior', size=256, states={'done': [('readonly', True)]})
    family_business = fields.Char(
        'Negocio familiar', size=256, states={'done': [('readonly', True)]})
    family_income = fields.Float(
        'Ingresos familiares', states={'done': [('readonly', True)]})
    gender = fields.Selection(
        [('m', 'Maculino'), ('f', 'Femenino'), ('o', 'Otros')], 'Género',
        required=True, states={'done': [('readonly', True)]})
    student_id = fields.Many2one(
        'op.student', 'Estudiante', states={'done': [('readonly', True)]})
    nbr = fields.Integer('No de Admisión', readonly=True)
    register_id = fields.Many2one(
        'op.admission.register', 'Registro de admisión', required=True)
    partner_id = fields.Many2one('res.partner', 'Partner')
    is_student = fields.Boolean('Ya es estudiante')
    fees_term_id = fields.Many2one('op.fees.terms', 'Plazo de honorarios')
    ...
    def open_student(self):
        form_view = self.env.ref('crmjj_core.view_op_student_form')
        tree_view = self.env.ref('crmjj_core.view_op_student_tree')
        value = {
