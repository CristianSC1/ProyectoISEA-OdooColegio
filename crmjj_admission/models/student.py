...
class OpStudentFeesDetails(models.Model):
    ...
    _description = 'Detalles de las tarifas de los estudiantes'

    fees_line_id = fields.Many2one('op.fees.terms.line', 'Linea de Cuotas')
    invoice_id = fields.Many2one('account.invoice', 'Factura')
  
    amount = fields.Float('Cuotas de Monto')
    date = fields.Date('Fecha de envío')
    product_id = fields.Many2one('product.product', 'Producto')
    student_id = fields.Many2one('op.student', 'Estudiante')
    state = fields.Selection([
        ('draft', 'Borrador'), ('invoice', 'Factura creada')], 'Estado')
    invoice_state = fields.Selection([
        ('draft', 'Borrador'), ('proforma', 'Pro-forma'),
        ('proforma2', 'Pro-forma'), ('open', 'Abierto'),
        ('paid', 'Pagado'), ('cancel', 'Cancelado')], 'Estado',
