...
class OpFaculty(models.Model):
    ...
    last_name = fields.Char('Segundo apellido', size=128, required=True)
    birth_date = fields.Date('Fecha de nacimiento')
    
    gender = fields.Selection(
        [('male', 'Masculino'), ('female', 'Femenino')], 'Género', required=True)
    ...
    last_login = fields.Datetime(
        'Última conexión', related='partner_id.user_id.login_date',
        readonly=1)
    faculty_subject_ids = fields.Many2many('op.subject', string='Subject(s)')
    emp_id = fields.Many2one('hr.employee', 'Empleado')
